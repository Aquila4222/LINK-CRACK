...
public class RemoteBulletAttack : IState<RemoteBulletState,RemoteBulletEnemy>
{
    ...
    public void OnState()
    {
        ...
        {
            bulletDirectionSpeed = (Context.targetTransform.position  - Context.bulletGenerateTransform.position).normalized * bulletSpeed;
            
            Context.ShootBullet(Context.bulletGenerateTransform.position, bulletDirectionSpeed);
            attacked = true;
        }
